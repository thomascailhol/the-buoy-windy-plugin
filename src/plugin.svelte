...
<script lang="ts">
    ...
    function createMarker(buoy: BuoySummary) {
        ...
        const heightLabel = formatHeightShort(
        ...
        const icon = L.divIcon({
            ...
            html: `<div class="buoy-marker__badge" style="--badge-color:${color};">
                ${arrowHtml}
                <span class="buoy-marker__text">${heightLabel}</span>
            </div>`,
            iconSize: [60, 24],
            iconAnchor: [30, 12],
        });
    ...
    function buildBuoyPopupContent(buoy: BuoySummary) {
        ...
        stats.appendChild(createStat('Height', formatHeight(lastReading?.significient_height, lastReading?.unit)));
        stats.appendChild(createStat('Hmax', formatHeight(lastReading?.maximum_height, lastReading?.unit)));
        stats.appendChild(createStat('Period', formatPeriod(lastReading?.period)));
        stats.appendChild(createStat('Direction', formatDirection(lastReading?.direction)));
    ...
    function formatHeightShort(value?: number | null, unit?: string | null) {
        const formatted = formatHeight(value, unit);
    ...
    function formatRelativeTime(time?: string | null) {
        ...
        return `${hours}h${minutes ? ` ${minutes}m` : ''} ago`;
    }
